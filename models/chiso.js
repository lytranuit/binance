  ...
  },
  {
    methods: {
      setIndicator: async function () {
        ...
        // console.log(candle1);
        var is_doji =
          technical.dragonflydoji(candle1) ||
          technical.gravestonedoji(candle1) ||
          technical.doji(candle1);
      ...
      get_candles: async function () {
        ...
        var time_current =
          Math.floor(moment().valueOf() / self.time) * self.time;
        var last_time_final = time_current - self.time;
        if (obj && obj[last_time_final] && obj[last_time_final].isFinal)
          ///// CANDLES DA HOAN THANH
          return obj;
        else if (obj) {
          //// LAY TIME TIEP THEO TRONG CANDLES
          if (obj[time_current]) delete obj[time_current];
          ...
          if (self.type != "5m") {
            sql =
              "SELECT a.*,b.close,c.open FROM(SELECT symbol,FLOOR(TIMESTAMP / " +
              self.time +
              ") * " +
              self.time +
              " AS 'timestamp',MIN(is_Final) as is_Final,SUM(`volume`) AS volume,MAX(high) AS high,MIN(low) AS low,MIN(TIMESTAMP) AS 'min_row',MAX(TIMESTAMP) AS 'max_row' FROM`candles` WHERE `symbol` = '" +
              self.symbol +
              "' GROUP BY symbol,`timestamp`) AS a JOIN candles AS b ON a.symbol = b.`symbol` AND a.max_row = b.`timestamp` JOIN candles AS c ON a.symbol = c.`symbol` AND a.min_row = c.`timestamp` WHERE a.timestamp >= '" +
              last_time_final +
              "' ORDER BY a.timestamp ASC";
          }
        } else {
          //// LAY LAY TOAN BO CANDLES
          var sql =
            "SELECT * FROM candles WHERE symbol = '" +
            self.symbol +
            "' ORDER BY timestamp ASC";
          if (self.type != "5m") {
            sql =
              ...
              " AS 'timestamp',MIN(is_Final) as is_Final,SUM(`volume`) AS volume,MAX(high) AS high,MIN(low) AS low,MIN(TIMESTAMP) AS 'min_row',MAX(TIMESTAMP) AS 'max_row' FROM`candles` WHERE `symbol` = '" +
              self.symbol +
              "' GROUP BY symbol,`timestamp`) AS a JOIN candles AS b ON a.symbol = b.`symbol` AND a.max_row = b.`timestamp` JOIN candles AS c ON a.symbol = c.`symbol` AND a.min_row = c.`timestamp` ORDER BY a.timestamp ASC";
            // console.log(sql)
          }
        }
        ...
        return mysql
          ...
          .then(function (conn) {
            var result = conn.query(
              "INSERT INTO candles (`" +
                keys.join("`,`") +
                "`) VALUES ? ON DUPLICATE KEY UPDATE is_Final = 1,close = VALUES(close),high = VALUES(high),low = VALUES(low),volume = VALUES(volume)",
              [values]
            );
            conn.end();
            return result;
