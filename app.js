      ...
      binance.prices((error, ticker) => {
        ...
        for (var i in ticker) {
          ...
          if (primary == "" || ignoreCoin.indexOf(alt) != -1)
            //
            continue;
          if (trackCoin.length > 0 && trackCoin.indexOf(alt) == -1) continue;
          var candles_1h = {};
          var candles_5m = {};
          try {
            candles_1h = jsonfile.readFileSync(
              "./candles/indicator_1h/" + market + ".json"
            );
          } catch (readOrJsonErr) {
            candles_1h = {};
          }
          try {
            candles_5m = jsonfile.readFileSync(
              "./candles/indicator_5m/" + market + ".json"
            );
          } catch (readOrJsonErr) {
            candles_5m = {};
          }
          ...
          if (global.chienLuocMua) obj.chienLuocMua = global.chienLuocMua;
          if (global.chienLuocBan) obj.chienLuocBan = global.chienLuocBan;
          if (global.minGain) obj.minGain = global.minGain;
        ...
        binance.websockets.trades(array_market, (trades) => {
          ...
          if (
            markets[symbol] &&
            markets[symbol]["indicator_1h"] &&
            markets[symbol]["indicator_5m"]
          ) {
            if (maker) {
              markets[symbol].trades.asks.push({
                price: price,
                quantity: quantity,
              });
              ...
              markets[symbol].trades.bids.push({
                price: price,
                quantity: quantity,
              });
